      ...
      <div class="container exp-section text-center text-md-start" id="exp">
        ...
        <div class="row">
          ...
          <div class="exp-col col-md-6 mb-5 d-flex flex-column">
            <div class="rounded-circle text-white text-center mb-4 mx-auto mx-md-0"><b>{{++$key}}</b></div>
            <div class="d-flex justify-content-center">
              <h3 class="mb-3">{{$exp->name}}</h3>
              @if(Session::get('Username') != NULL)
              <button class="btn btn-sm btn-warning mx-3 mb-3" data-bs-toggle="modal" data-bs-target="#editExp{{$exp->ID}}"">
                <i class="fa-regular fa-pen-to-square"></i>
              </button>
              <button class="btn btn-sm btn-danger mb-3" data-bs-toggle="modal" data-bs-target="#delExp{{$exp->ID}}">
          ...
          <div class="modal fade" id="delExp{{$exp->ID}}"aria-hidden="true" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content rounded-0">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <form method="POST" action="{{ url('/exp/'.$exp->ID) }}">
                            @csrf
        ...
        <div class="modal fade" id="editExp{{$exp->ID}}"aria-hidden="true" style="display: none;">
          <div class="modal-dialog">
            <div class="modal-content rounded-0">
              <div class="modal-header">
              ...
              <div class="modal-body">
                <form method="POST" action="{{ url('/exp/'.$exp->ID) }}">
                  ...
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control rounded-0" name="name" value="{{$exp->name}}" required>
                  </div>
                
                  <div class="mb-3">
                      <label for="detail" class="col-form-label">Experience Detail</label>
                      <input type="text" class="form-control rounded-0" name="detail" value="{{$exp->detail}}" required>
                  </div>
      ...
      <div class="container-fluid bg-dark bg-gradient pt-4 pb-5 px-5 text-center text-white ">
        <div class="container working-with">
          ...
          <div class="work-list">
            ...
            <div class="d-flex justify-content-center">
              <p class="mb-0 mt-1">{{$skill->name}}</p>
            @if(Session::get('Username') != NULL)
              <button class="btn btn-sm btn-outline-light ms-2" data-bs-toggle="modal" data-bs-target="#delSkill{{$skill->ID}}">
                <i class="fa-regular fa-trash-can"></i>
              </button>
              <div class="modal fade" id="delSkill{{$skill->ID}}"aria-hidden="true" style="display: none;">
                <div class="modal-dialog">
                    <div class="modal-content rounded-0">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <form method="POST" action="{{ url('/skill/'.$skill->ID) }}">
                                @csrf
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="button submit"class="btn btn-danger rounded-0">Delete</button>
                            </form>
